...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (typeof credit !== 'number' || credit <= 0) {
            return NextResponse.json(
                { success: false, message: 'Valid positive credit value is required' },
                { status: 400 }
            );
        }
        ...
        if (currentBalance < credit) {
            return NextResponse.json(
                { success: false, message: 'Insufficient credit balance' },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: false, message: 'Failed to update credit balance' },
            { status: 500 }
        );
    }
}